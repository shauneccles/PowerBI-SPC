...
export default function pprimeLimits(args: controlLimitsArgs): controlLimitsObject {
  ...
  const denominators: readonly number[] = args.denominators!; // Sample sizes (always provided for P'-chart)
  ...
  for (let i = 0; i < n; i++) {
    ...
    targets[i] = cl;                                   // Centreline: p̄
    ll99[i] = Math.max(0, cl - 3 * sigma);             // LCL: max(0, p̄ - 3σ)
    ll95[i] = Math.max(0, cl - 2 * sigma);             // 2σ lower: max(0, p̄ - 2σ)
    ll68[i] = Math.max(0, cl - 1 * sigma);             // 1σ lower: max(0, p̄ - σ)
    ul68[i] = Math.min(1, cl + 1 * sigma);             // 1σ upper: min(1, p̄ + σ)
    ul95[i] = Math.min(1, cl + 2 * sigma);             // 2σ upper: min(1, p̄ + 2σ)
    ul99[i] = Math.min(1, cl + 3 * sigma);             // UCL: min(1, p̄ + 3σ)
  }
