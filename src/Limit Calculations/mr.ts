...
export default function mrLimits(args: controlLimitsArgs): controlLimitsObject {
  ...
  const values = new Array<number>(n_mr);
  const { targets, ll99, ll95, ll68, ul68, ul95, ul99 } = createLimitArrays(n_mr);
  ...
  for (let i = 0; i < n_mr; i++) {
    values[i] = consec_diff[i];
    targets[i] = cl;                        // Centreline: MR̄
    ll99[i] = 0;                            // LCL: 0
    ll95[i] = 0;                            // 2σ lower: 0
    ll68[i] = 0;                            // 1σ lower: 0
    ul68[i] = (3.267 / 3) * 1 * cl;         // 1σ upper: 1.089 × MR̄
    ul95[i] = (3.267 / 3) * 2 * cl;         // 2σ upper: 2.178 × MR̄
    ul99[i] = 3.267 * cl;                   // UCL: 3.267 × MR̄ (D4 constant)
  }
